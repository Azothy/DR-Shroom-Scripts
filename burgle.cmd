...
if (matchre("$scriptlist", "uber\.cmd") then var pawn NO 
...
HIDE:
     ...
     pause .001
     pause .001
     matchre HIDE_PLAYING ^You are a bit too busy performing
     matchre BURGLE ^Roundtime\:?|^\[Roundtime\:?|^\(Roundtime\:?|already hidden
     send hide
     matchwait

HIDE_PLAYING:
     send stop play
     waitforre ^You stop playing|^In the name of love\?
     goto HIDE
...
BURGLETIMER:
	...
	if ("%item3" != "NULL") then 
          {
               put #echo >log #C59BFE * Burgled %successful items
               put #echo >log #C59BFE * %item1, %item2, %item3
               goto END
          }
     if ("%item2" != "NULL") then 
          {
               put #echo >log #C59BFE * Burgled %successful items:
               put #echo >log #C59BFE * %item1, %item2
               goto END
          }
     if ("%item1" != "NULL") then 
          {
               put #echo >log #C59BFE * Burgled %successful item: %item1
               goto END
          }
     if ("%item1" = "NULL") then 
          {
               put #echo >log #C59BFE * Burgled %successful items: No successful searches
               goto END
          }
	goto END
...
PAWN:
	var last pawn
     ...
     if matchre("%home", "(?i)(NO|NULL|0|OFF)") then goto DONE_3
	...
	if !matchre("$righthand", "Empty") then 
          {
			   ...
			   if (!matchre("$righthandnoun", "(%keep|rope|lockpick)") && (matchre("%trashall", "(?i)(YES|ON)"))||matchre("$righthandnoun", "%trashitems")) then gosub DROP right
			   else gosub PUTLOOT put my $righthandnoun in my %pack
			   if ((%nofit = 1) && !matchre("$righthandnoun", "(%keep|rope|lockpick)")) then 
			   {
				gosub DROP right
				var nofit 0
			   }			   
	...
	if !matchre("$lefthand", "Empty") then 
          {
			   ...
			   if (!matchre("$lefthandnoun", "(%keep|rope|lockpick)") && (matchre("%trashall", "(?i)(YES|ON)")) || matchre("$lefthandnoun", "%trashitems")) then gosub DROP left
			   else gosub PUTLOOT put my $lefthandnoun in my %pack
			   if ((%nofit = 1) && !matchre("$lefthandnoun", "(%keep|rope|lockpick)")) then 
			   {
				gosub DROP left
				var nofit 0
			   }			   
