...
###########################################
# CHANGELOG - Latest Update: 4/25/2020
...
if (matchre("%destination", "(ratha)") && matchre("$zoneid", "\b(1|30|42|47|61|66|67|90|99|107|108|116)\b")) then
     {
     if (("$game" = "DRF") || (%premium = 1)) then
               {
                    echo *** GOING TO FC
                    gosub TO_SEACAVE
                    gosub MOVE 2
...
CROSSING:
var label CROSSING
if (("$zoneid" = "116") && ("$game" = "DRX") && (%portal = 1)) then goto PORTAL_TIME
if (("$zoneid" = "67") && ("$game" = "DRX") && (%portal = 1)) then goto PORTAL_TIME
...
if ("$zoneid" = "47") then 
     {
          if (("$game" = "DRX") && (%portal = 1)) then goto PORTAL_TIME
          gosub MOVE 117
          gosub FERRYLOGIC
          pause
     }
...
if (("$zoneid" = "40") && ("$game" = "DRX") && (%portal = 1)) then goto PORTAL_TIME
...
if (("$zoneid" = "30") && ("$game" = "DRX") && (%portal = 1)) then goto PORTAL_TIME
...
if ("$zoneid" = "7") then gosub MOVE 349
if ("$zoneid" = "112") then gosub MOVE 112
if (("$zoneid" = "1") && ("$game" = "DRX") && (%portal = 1)) then goto PORTAL_TIME
if ("$zoneid" = "1") then
          {
              if "$guild" = "Thief" then
                  {
...
if (("$zoneid" = "68") && matchre("%shardcitizen", "(?i)no")) then gosub MOVE 15
if (("$zoneid" = "67") && ("$game" = "DRX") && (%portal = 1)) then goto PORTAL_TIME
if (("$zoneid" = "67") && ("$guild" = "Thief")) then
          {
              gosub MOVE 566
...
if (("$zoneid" = "47") && ("$game" = "DRX") && (%portal = 1)) then goto PORTAL_TIME
...
if (("$zoneid" = "40") && ("$game" = "DRX") && (%portal = 1) && !matchre("%detour", "(?i)(el'bain|lang|rakash)") then goto PORTAL_TIME
...
ONFERRY:
  ...
  send hide
  pause
  if matchre("$roomobjs","the beach") && matchre("%destination", "ratha?") then goto OFFTHERIDE
  if matchre("$roomobjs","a ladder") && !matchre("%destination", "ratha?") then goto OFFTHERIDE
  if ("$guild" = "Necromancer") then
     {
...
GET_PASSPORT:
    ...
    echo ===============
    if ("$zoneid" = "40") then gosub MOVE theren
    pause 0.2
    gosub MOVE passport
    pause 0.5
    pause 0.2
...
TO_SEACAVES:
     if ("$zoneid" = "67") then gosub MOVE east
     if ("$zoneid" = "127") then gosub MOVE south
     if ("$zoneid" = "124") then gosub MOVE hib
     if ("$zoneid" = "112") then gosub MOVE leth
     if ("$zoneid" = "4") then gosub MOVE cross
     if ("$zoneid" = "7") then gosub MOVE cross
     if ("$zoneid" = "67") then gosub MOVE east
     if ("$zoneid" = "42") then gosub MOVE gate
     pause 0.1
     gosub MOVE portal
     pause 0.5
...
CROSS_PORTAL:
     if ("$zoneid" = "1") then
          {
               if ($roomid != 484) then gosub MOVE 484
...
AESRY_PORTAL:
     if ("$zoneid" = "99") then
          {
               if ($roomid != 115) then gosub MOVE 115
               ...
               if matchre("%destination", "\b(grani?t?e?|garg?o?y?l?e?|spir?e?|horse?c?l?a?n?|fayr?i?n?s?|steel?c?l?a?w?|cori?k?s?|ada?n?f?|ylo?n?o?|wyve?r?n?|aing?h?a?z?a?l?|rave?n?s?|boar?c?l?a?n?|fan?g?|cov?e?)\b") then goto ILITHI
          }
## SHARD PORTAL ENTRANCE Zone 67 Room 455
SHARD_PORTAL:
     if ("$zoneid" = "67") then
          {
               if ($roomid != 455) then gosub MOVE 455
...
MERKRESH_PORTAL:
     if ("$zoneid" = "107") then
          {
               if ($roomid != 273) then gosub MOVE 273
...
RIVERHAVEN_PORTAL:
     if ("$zoneid" = "30") then
          {
               if ($roomid != 331) then gosub MOVE 331
...
RATHA_PORTAL:
     if ("$zoneid" = "90") then
          {
               if ($roomid != 468) then gosub MOVE 468
...
ELBAINS_PORTAL:
     if ("$zoneid" = "40") then
          {
               if ($roomid != 254) then gosub MOVE 254
...
MUSPARI_PORTAL:
     if ("$zoneid" = "47") then
          {
               if ($roomid != 97) then gosub MOVE 97
...
HIB_PORTAL:
     ...
     if ("$zoneid" = "126") then gosub MOVE boar
     put east
     pause 0.5
     pause 0.2
     if ("$zoneid" = "116") then
          {
               if ($roomid != 188) then gosub MOVE 188
